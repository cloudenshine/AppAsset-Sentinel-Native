...
| **R1 可信迁移** | 🟡 跨卷已验收，真实应用迁移后验收未跑 | W06/W07/W08 完成；**跨物理盘迁移已在真实两个卷上验收通过**（见下节）。真实应用"迁后重启推理"需用户在场维护窗口，未执行。R1 档位仍需显式 `--profile=r1`。 |
...
- **CLI / MCP 接入面**：未实现，因此"API/CLI/GUI 同一授权政策"只覆盖了前两者。
